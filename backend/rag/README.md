...
> Domain: `backend/rag`. ingest->chunk->embed->pgvector->retrieve->rerank->generate (08) — tenant filter at query time. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
RAG Pipeline tồn tại để ingest->chunk->embed->pgvector->retrieve->rerank->generate (08) — tenant filter at query time. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi rag; expose interface ổn định cho caller.
...
- Chi tiết: `docs/architecture/rag.md`.
...
## Related
- Plan: `./rag-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
