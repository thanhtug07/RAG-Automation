# Workflows & Orchestration — Implementation Plan

> Kind: `backend` | Domain: `workflows` | Ưu tiên: Foundation>Auth>Agent>Planner>Workflow>CrewAI>Execution>RAG>Memory>Tools>LLM>Context>Policy>Events>Observability>Evaluation>Frontend. Chỉ skeleton/config, không full implementation.

## 1. Objective
Đưa `backend/workflows` (workflow dsl (29-workflow-dsl.md), dag execution — sequential/hierarchical, hitl approval) lên Modular Monolith chạy được tối thiểu (healthcheck + contract test xanh), giữ chỗ cho implementation đầy đủ sau.
...
## 5. Dependencies
- Upstream: API Gateway. Hạ tầng: Postgres/Redis sẵn qua `infrastructure/`.
...
## 7. Flow
1. Caller gọi interface của `workflows` (REST/SSE/import).
...
## 13. API
- Backend: `POST /api/v1/workflows` (nếu áp dụng), `GET /api/v1/workflows`, SSE cho run events. Frontend: typed client trong `frontend/workflows/` hoặc `frontend/api/`. Version `/v1`, pagination cursor/limit.

## 14. Event
- Publish: `workflows.completed|failed` với `{run_id, tenant_id, ts, payload_ref}`; persist Postgres trước khi pub Redis (không mất event khi đổi broker).

## 15. Files Create
- `backend/workflows/README.md` (13 mục), `backend/workflows/workflows-plan.md` (21 mục này), `__init__.py`/`index.ts` stub, `interface.*` stub, `config.*` nếu cần, `tests/test_workflows_contract.*` skeleton.
...
## 17. Tests
- Contract test: import/interface tồn tại, stub trả đúng shape, event có `run_id/tenant_id`. Lệnh: `pytest backend/workflows -q` / `npm run test -- frontend/workflows` (skeleton xanh với fixture).
...
## 19. Observability
- Log JSON có `tenant_id/run_id`; counter `workflows_requests_total`, histogram latency; trace span per Flow step; dashboard skeleton ở `infrastructure/monitoring/`.
