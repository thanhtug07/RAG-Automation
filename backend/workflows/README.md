# Workflows & Orchestration — README

> Domain: `backend/workflows`. Workflow DSL (29-workflow-dsl.md), DAG execution — sequential/hierarchical, HITL approval. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Workflows & Orchestration tồn tại để workflow dsl (29-workflow-dsl.md), dag execution — sequential/hierarchical, hitl approval. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi workflows; expose interface ổn định cho caller.
...
## Related
- Plan: `./workflows-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
