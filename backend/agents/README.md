# Agents Domain — README

> Domain: `backend/agents`. lookup/analysis/policy/manager from agents.py -> Blueprint registry — Agent Blueprint JSON schema, lifecycle. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Agents Domain tồn tại để lookup/analysis/policy/manager from agents.py -> blueprint registry — agent blueprint json schema, lifecycle. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi agents; expose interface ổn định cho caller.
...
## Related
- Plan: `./agents-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
