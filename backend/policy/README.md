# Policy Engine — README

> Domain: `backend/policy`. OPA-style checks before execution — allow/deny, HITL, audit. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Policy Engine tồn tại để opa-style checks before execution — allow/deny, hitl, audit. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi policy; expose interface ổn định cho caller.
...
## Related
- Plan: `./policy-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
