...
> Domain: `backend/events`. RunEvent persist Postgres + Redis pub/sub -> SSE (16) — event schema, ordering. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Event Bus tồn tại để runevent persist postgres + redis pub/sub -> sse (16) — event schema, ordering. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi events; expose interface ổn định cho caller.
...
## Related
- Plan: `./events-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
