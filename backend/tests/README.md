...
> Domain: `backend/tests`. pytest, contract tests for Runtime/Adapter — FakeRuntime, fixtures. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Backend Tests tồn tại để pytest, contract tests for runtime/adapter — fakeruntime, fixtures. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi tests; expose interface ổn định cho caller.
...
## Related
- Plan: `./tests-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
