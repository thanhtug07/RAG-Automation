# Backend API Gateway — README

> Domain: `backend/api`. REST/SSE entry: User>API>Planner>Blueprint>ExecutionPlan — FastAPI routers, SSE, versioning, pagination. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Backend API Gateway tồn tại để rest/sse entry: user>api>planner>blueprint>executionplan — fastapi routers, sse, versioning, pagination. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi api; expose interface ổn định cho caller.
...
## Related
- Plan: `./api-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
