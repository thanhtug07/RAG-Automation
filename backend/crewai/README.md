# CrewAI Adapter — README

> Domain: `backend/crewai`. Execution Manager/Planner -> CrewAI Adapter -> Agents — AgentRuntime Interface, FakeRuntime for tests. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
CrewAI Adapter tồn tại để execution manager/planner -> crewai adapter -> agents — agentruntime interface, fakeruntime for tests. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi crewai; expose interface ổn định cho caller.
...
## Related
- Plan: `./crewai-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
