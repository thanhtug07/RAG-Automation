...
> Domain: `backend/database`. Postgres system of record + pgvector MVP (14) — migrations, RLS, JSONB. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Database tồn tại để postgres system of record + pgvector mvp (14) — migrations, rls, jsonb. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi database; expose interface ổn định cho caller.
...
- Chi tiết: `docs/architecture/database.md`.
...
## Related
- Plan: `./database-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
