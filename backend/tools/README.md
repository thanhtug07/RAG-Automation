...
> Domain: `backend/tools`. OrderSearch/ProductSearch/RevenueAnalysis/PolicySearch -> Tool Registry (10) — JSON tools, RBAC, audit. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Tool System tồn tại để ordersearch/productsearch/revenueanalysis/policysearch -> tool registry (10) — json tools, rbac, audit. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi tools; expose interface ổn định cho caller.
...
## Related
- Plan: `./tools-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
