...
> Domain: `backend/llm`. multi-provider via gateway (11), budget control (33) — routing, fallback, cost tracking. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
LLM Gateway tồn tại để multi-provider via gateway (11), budget control (33) — routing, fallback, cost tracking. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi llm; expose interface ổn định cho caller.
...
- Chi tiết: `docs/architecture/llm.md`.
...
## Related
- Plan: `./llm-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
