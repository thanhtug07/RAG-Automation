"""PolicyCrew: collect policy excerpts -> grounded explanation."""
...
def build(model: str | None = None) -> Crew:
    """Construct the policy crew (no kickoff)."""
    llm = model or default_model()
    agents_cfg, tasks_cfg = _cfg("agents.yaml"), _cfg("tasks.yaml")
    tools = {"researcher": [DocumentSearchTool()], "rag_retriever": []}
