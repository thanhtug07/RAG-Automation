...
> Domain: `infrastructure/vector`. pgvector MVP -> Qdrant path — re-index plan. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Vector DB tồn tại để pgvector mvp -> qdrant path — re-index plan. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi vector; expose interface ổn định cho caller.
...
## Related
- Plan: `./vector-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
