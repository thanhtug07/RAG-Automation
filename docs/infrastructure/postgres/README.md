...
> Domain: `infrastructure/postgres`. PG16 + pgvector, RLS — init scripts, migrations. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Postgres tồn tại để pg16 + pgvector, rls — init scripts, migrations. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi postgres; expose interface ổn định cho caller.
...
## Related
- Plan: `./postgres-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
