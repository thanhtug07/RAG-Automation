...
- Code cũ giữ nguyên giá trị: `backend/crewai/agents/legacy_agents.py` (lookup/analysis/policy/manager), `backend/crewai/agents/legacy_crew.py` (hierarchical crew), `tools/*_tools.py` (4 tools), `data/*.json` (seed).
...
tools/ data/ scripts/cli.py + backend/crewai/agents/legacy_*.py   # code cũ (giữ, không phá)
```
...
## 6. Agent
- Static/Configurable ưu tiên, Dynamic có kiểm soát (Validation Pipeline). Blueprint JSON schema. Xem `docs/architecture/agent.md` + `backend/crewai/agents/`.
...
3. `python scripts/cli.py` (CLI cũ vẫn chạy)
...
## 17. Testing
- `python -m compileall backend tools scripts`
