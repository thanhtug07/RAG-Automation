...
> Kind: `backend` | Domain: `llm` | Ưu tiên: Foundation>Auth>Agent>Planner>Workflow>CrewAI>Execution>RAG>Memory>Tools>LLM>Context>Policy>Events>Observability>Evaluation>Frontend. Chỉ skeleton/config, không full implementation.

## 1. Objective
Đưa `backend/llm` (multi-provider via gateway (11), budget control (33) — routing, fallback, cost tracking) lên Modular Monolith chạy được tối thiểu (healthcheck + contract test xanh), giữ chỗ cho implementation đầy đủ sau.
...
## 7. Flow
1. Caller gọi interface của `llm` (REST/SSE/import).
...
## 13. API
- Backend: `POST /api/v1/llm` (nếu áp dụng), `GET /api/v1/llm`, SSE cho run events. Frontend: typed client trong `frontend/llm/` hoặc `frontend/api/`. Version `/v1`, pagination cursor/limit.

## 14. Event
- Publish: `llm.completed|failed` với `{run_id, tenant_id, ts, payload_ref}`; persist Postgres trước khi pub Redis (không mất event khi đổi broker).

## 15. Files Create
- `backend/llm/README.md` (13 mục), `backend/llm/llm-plan.md` (21 mục này), `__init__.py`/`index.ts` stub, `interface.*` stub, `config.*` nếu cần, `tests/test_llm_contract.*` skeleton.
...
## 17. Tests
- Contract test: import/interface tồn tại, stub trả đúng shape, event có `run_id/tenant_id`. Lệnh: `pytest backend/llm -q` / `npm run test -- frontend/llm` (skeleton xanh với fixture).
...
## 19. Observability
- Log JSON có `tenant_id/run_id`; counter `llm_requests_total`, histogram latency; trace span per Flow step; dashboard skeleton ở `infrastructure/monitoring/`.
