...
> Domain: `backend/context`. context assembly, token budget (30) — prompt templates, compression. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Context Engineering tồn tại để context assembly, token budget (30) — prompt templates, compression. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi context; expose interface ổn định cho caller.
...
- Chi tiết: `docs/architecture/context.md`.
...
## Related
- Plan: `./context-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
