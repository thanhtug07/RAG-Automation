# Evaluation & Testing — README

> Domain: `backend/evaluation`. golden sets, RAG eval, agent eval (20) — pass thresholds. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Evaluation & Testing tồn tại để golden sets, rag eval, agent eval (20) — pass thresholds. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi evaluation; expose interface ổn định cho caller.
...
- Chi tiết: `docs/architecture/evaluation.md`.
...
## Related
- Plan: `./evaluation-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
