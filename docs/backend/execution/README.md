# Execution / Runs — README

> Domain: `backend/execution`. Run execution system (15), Redis queue + workers — Run, RunStep, RunEvent, idempotency. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Execution / Runs tồn tại để run execution system (15), redis queue + workers — run, runstep, runevent, idempotency. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi execution; expose interface ổn định cho caller.
...
## Related
- Plan: `./execution-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
