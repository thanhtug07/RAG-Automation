# Auth & Multi-tenancy — README

> Domain: `backend/auth`. JWT + organization_id + RLS — login, refresh, API keys, RBAC. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Auth & Multi-tenancy tồn tại để jwt + organization_id + rls — login, refresh, api keys, rbac. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi auth; expose interface ổn định cho caller.
...
## Related
- Plan: `./auth-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
