...
> Domain: `backend/memory`. short-term (Redis) + long-term (Postgres+vector) (12) — consent, TTL, per-tenant isolation. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Memory System tồn tại để short-term (redis) + long-term (postgres+vector) (12) — consent, ttl, per-tenant isolation. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi memory; expose interface ổn định cho caller.
...
- Chi tiết: `docs/architecture/memory.md`.
...
## Related
- Plan: `./memory-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
