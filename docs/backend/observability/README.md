...
> Domain: `backend/observability`. traces/metrics/logs, OpenTelemetry — run_id trace, dashboards. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Observability tồn tại để traces/metrics/logs, opentelemetry — run_id trace, dashboards. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi observability; expose interface ổn định cho caller.
...
- Chi tiết: `docs/architecture/observability.md`.
...
## Related
- Plan: `./observability-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
