...
> Domain: `backend/planner`. Request Analyzer -> Task Planner -> Agent Builder -> Policy check — ExecutionPlan DAG generation. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Planner tồn tại để request analyzer -> task planner -> agent builder -> policy check — executionplan dag generation. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi planner; expose interface ổn định cho caller.
...
- Chi tiết: `docs/architecture/planner.md`.
...
## Related
- Plan: `./planner-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
