# Frontend Services — Implementation Plan

> Kind: `frontend` | Domain: `services` | Ưu tiên: Foundation>Auth>Agent>Planner>Workflow>CrewAI>Execution>RAG>Memory>Tools>LLM>Context>Policy>Events>Observability>Evaluation>Frontend. Chỉ skeleton/config, không full implementation.

## 1. Objective
Đưa `frontend/services` (business logic layer — mapping dtos) lên Modular Monolith chạy được tối thiểu (healthcheck + contract test xanh), giữ chỗ cho implementation đầy đủ sau.
...
## 7. Flow
1. Caller gọi interface của `services` (REST/SSE/import).
...
## 13. API
- Backend: `POST /api/v1/services` (nếu áp dụng), `GET /api/v1/services`, SSE cho run events. Frontend: typed client trong `frontend/services/` hoặc `frontend/api/`. Version `/v1`, pagination cursor/limit.

## 14. Event
- Publish: `services.completed|failed` với `{run_id, tenant_id, ts, payload_ref}`; persist Postgres trước khi pub Redis (không mất event khi đổi broker).

## 15. Files Create
- `frontend/services/README.md` (13 mục), `frontend/services/services-plan.md` (21 mục này), `__init__.py`/`index.ts` stub, `interface.*` stub, `config.*` nếu cần, `tests/test_services_contract.*` skeleton.
...
## 17. Tests
- Contract test: import/interface tồn tại, stub trả đúng shape, event có `run_id/tenant_id`. Lệnh: `pytest backend/services -q` / `npm run test -- frontend/services` (skeleton xanh với fixture).
...
## 19. Observability
- Log JSON có `tenant_id/run_id`; counter `services_requests_total`, histogram latency; trace span per Flow step; dashboard skeleton ở `infrastructure/monitoring/`.
