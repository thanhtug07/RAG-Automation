...
> Domain: `frontend/routing`. Next.js App Router structure — protected routes, deep-link runId. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Routing tồn tại để next.js app router structure — protected routes, deep-link runid. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi routing; expose interface ổn định cho caller.
...
## Related
- Plan: `./routing-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
