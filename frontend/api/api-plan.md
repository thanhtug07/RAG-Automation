# Frontend API Client — Implementation Plan

> Kind: `frontend` | Domain: `api` | Ưu tiên: Foundation>Auth>Agent>Planner>Workflow>CrewAI>Execution>RAG>Memory>Tools>LLM>Context>Policy>Events>Observability>Evaluation>Frontend. Chỉ skeleton/config, không full implementation.

## 1. Objective
Đưa `frontend/api` (typed client for backend rest/sse — fetch wrapper, error mapping) lên Modular Monolith chạy được tối thiểu (healthcheck + contract test xanh), giữ chỗ cho implementation đầy đủ sau.
...
## 15. Files Create
- `frontend/api/README.md` (13 mục), `frontend/api/api-plan.md` (21 mục này), `__init__.py`/`index.ts` stub, `interface.*` stub, `config.*` nếu cần, `tests/test_api_contract.*` skeleton.
