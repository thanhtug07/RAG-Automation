# Frontend API Client — README

> Domain: `frontend/api`. typed client for backend REST/SSE — fetch wrapper, error mapping. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Frontend API Client tồn tại để typed client for backend rest/sse — fetch wrapper, error mapping. Giữ boundary rõ để tách service sau này không rewrite logic.
...
- Vị trí: User -> Pages -> Services -> Backend API.
