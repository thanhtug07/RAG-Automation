...
> Domain: `frontend/realtime`. EventSource for RunEvent stream — reconnect, resume via Last-Event-ID. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Realtime (SSE) tồn tại để eventsource for runevent stream — reconnect, resume via last-event-id. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi realtime; expose interface ổn định cho caller.
...
## Related
- Plan: `./realtime-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
