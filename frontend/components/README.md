...
> Domain: `frontend/components`. Run Monitor, Blueprint editor, Tool cards — storybook-style props. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Components tồn tại để run monitor, blueprint editor, tool cards — storybook-style props. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi components; expose interface ổn định cho caller.
...
## Related
- Plan: `./components-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
