...
> Domain: `frontend/state`. server-state + client-state split — React Query + zustand. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
State Management tồn tại để server-state + client-state split — react query + zustand. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi state; expose interface ổn định cho caller.
...
## Related
- Plan: `./state-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
