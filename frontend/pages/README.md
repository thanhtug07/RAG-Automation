...
> Domain: `frontend/pages`. route pages: chat, runs, blueprints, tools — empty/error states. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Pages tồn tại để route pages: chat, runs, blueprints, tools — empty/error states. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi pages; expose interface ổn định cho caller.
...
## Related
- Plan: `./pages-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
