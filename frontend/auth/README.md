# Frontend Auth — README

> Domain: `frontend/auth`. login, token storage, refresh — httpOnly cookie, guards. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Frontend Auth tồn tại để login, token storage, refresh — httponly cookie, guards. Giữ boundary rõ để tách service sau này không rewrite logic.
