...
> Domain: `frontend/tests`. playwright + vitest — MSW mocks. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Frontend Tests tồn tại để playwright + vitest — msw mocks. Giữ boundary rõ để tách service sau này không rewrite logic.
