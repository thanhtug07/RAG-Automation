...
> Domain: `infrastructure/redis`. queue+cache+pubsub — persistence config. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Redis tồn tại để queue+cache+pubsub — persistence config. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi redis; expose interface ổn định cho caller.
...
## Related
- Plan: `./redis-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
