...
> Domain: `infrastructure/monitoring`. Prometheus+Grafana+Loki/OTEL — red metrics, alerts. Architecture-first, Modular Monolith + Workers + Redis + Postgres + pgvector. Cấm microservices sớm.

## 1. Purpose
Monitoring tồn tại để prometheus+grafana+loki/otel — red metrics, alerts. Giữ boundary rõ để tách service sau này không rewrite logic.

## 2. Responsibilities
- Thực thi đúng phạm vi monitoring; expose interface ổn định cho caller.
...
## Related
- Plan: `./monitoring-plan.md`. Kiến trúc chi tiết: `docs/architecture/`.
